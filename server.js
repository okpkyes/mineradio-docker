...
const { browseDirectory, scanDirectory, getFileMeta, resolveMusicPath, LOCAL_FILE_MIME } = require('./src/fs-library');
...
function serveStatic(res, filePath) {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    if (ext === '.html') {
      // SPA 壳 / 首页：禁用缓存，避免「刷新仍是旧页」
      headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, proxy-revalidate';
      headers['Pragma'] = 'no-cache';
      headers['Expires'] = '0';
    } else {
      // 其它静态资源（js/css/图片/字体）：长效缓存，资源很少变动
...
// ---------- 本地文件服务（与 /music 扫描能力共用） ----------
const LOCAL_MUSIC_PLAYLIST_ID = 'local-music';
const LOCAL_MUSIC_PLAYLIST_NAME = '本地音乐';

/**
 * 按相对 MUSIC_ROOT 的路径流出本地音频/文件，支持 Range 206 部分内容。
 * 由 /api/local-file 与 /api/audio 的本地快捷通道共用。
 * @param {string} relPath 相对 MUSIC_ROOT 的路径（来自 ?path=）
 * @param {http.IncomingMessage} req
 * @param {http.ServerResponse} res
 */
async function serveLocalFile(relPath, req, res) {
  let resolved;
  try {
    resolved = resolveMusicPath(relPath);
  } catch (e) {
    res.writeHead(400, { 'Content-Type': 'text/plain; charset=utf-8' });
    res.end('Invalid path: ' + (e.code || e.message));
    return;
  }
  let stat;
  try {
  ...
  const contentType = LOCAL_FILE_MIME[path.extname(resolved.abs).toLowerCase()] || 'application/octet-stream';
  ...
  if (range) {
    ...
    const stream = fs.createReadStream(resolved.abs, { start, end });
...
/**
 * 确保「本地音乐」歌单存在（稳定 id = local-music），返回该歌单对象。
 * @returns {object} 歌单对象
 */
function ensureLocalMusicPlaylist() {
  let pl = localLibrary.state.playlists.find(p => p.id === LOCAL_MUSIC_PLAYLIST_ID);
  if (!pl) {
    localLibrary.createPlaylist(LOCAL_MUSIC_PLAYLIST_NAME, '扫描 ' + (process.env.MUSIC_ROOT || '/music') + ' 目录得到的本地音乐', LOCAL_MUSIC_PLAYLIST_ID);
    pl = localLibrary.state.playlists.find(p => p.id === LOCAL_MUSIC_PLAYLIST_ID);
  }
  return pl;
}

function localMusicRawSongsFromScan(scan) {
...
async function handleSongUrl(id, loginInfo, qualityPreference) {
  ...
  // 本地文件：直接返回本地文件服务地址，绕开网易云音源解析。
  const localKey = 'local:' + String(id || '');
  const localTrack = localLibrary.state.tracks[localKey];
  if (localTrack && localTrack.source === 'local' && localTrack.url) {
    console.log('[SongUrl] local file ->', localTrack.url);
    return { url: localTrack.url, playable: true, source: 'local', local: true, id: id, trial: false };
  }
  const requestedQuality = normalizeQualityPreference(qualityPreference);
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/config') {
    sendJSON(res, {
      ...
      name: APP_PACKAGE.name || 'mineradio',
      productName: APP_PACKAGE.productName || 'Mineradio',
      musicRoot: process.env.MUSIC_ROOT || '/music',
      hasToken: !!process.env.MINERADIO_LOCAL_FILE_TOKEN,
      maxScanFiles: parseInt(process.env.MAX_SCAN_FILES, 10) || 10000,
      mode: 'docker-nas',
    });
    return;
  }
  ...
  // ---------- 本地音乐：目录浏览 ----------
  if (pn === '/api/library/browse') {
    try {
      const relPath = url.searchParams.get('path') || '';
      const data = await browseDirectory(relPath);
      sendJSON(res, { ok: true, ...data });
    } catch (e) {
      sendJSON(res, { error: e.message, code: e.code }, 400);
    }
    return;
  }

  // ---------- 本地音乐：递归扫描并写入「本地音乐」歌单 ----------
  if (pn === '/api/library/scan') {
    try {
      const scan = await scanDirectory('');
      const playlist = ensureLocalMusicPlaylist();
      const rawSongs = localMusicRawSongsFromScan(scan);
      const result = localLibrary.replacePlaylistTracks(playlist.id, rawSongs);
      sendJSON(res, {
        ok: true,
        count: result.count,
        total: scan.total,
        dir: scan.dir,
        musicRoot: scan.musicRoot,
        truncated: scan.truncated,
        playlistId: playlist.id,
      });
    } catch (e) {
      console.error('[Scan]', e);
      sendJSON(res, { error: e.message, code: e.code }, 500);
    }
    return;
  }

  // ---------- 本地音乐：单文件元数据 ----------
  if (pn === '/api/library/meta') {
    try {
      const relPath = url.searchParams.get('path') || '';
      const data = await getFileMeta(relPath);
      sendJSON(res, { ok: true, ...data });
    } catch (e) {
      sendJSON(res, { error: e.message, code: e.code }, 400);
    }
  ...
  // ---------- 本地文件服务（按相对路径流出，支持 Range 206） ----------
  if (pn === '/api/local-file') {
    try {
      const relPath = url.searchParams.get('path') || '';
      await serveLocalFile(relPath, req, res);
    } catch (e) {
      console.error('[LocalFile]', e);
  ...
  if (pn === '/api/audio') {
    try {
      ...
      // 本地文件快捷通道：audioUrl 是内部 /api/local-file 地址，直接由文件系统流出（支持 Range）。
      if (/^\/api\/local-file(\?|$)/i.test(audioUrl)) {
        const inner = new URL(audioUrl, 'http://localhost');
        const relPath = inner.searchParams.get('path') || '';
        await serveLocalFile(relPath, req, res);
        return;
      }
      // 仅允许 http(s) 绝对地址走远程代理；相对路径 / 其他 scheme 一律 400 拦截
      // （防路径穿越 + 防非法 URL，确保 resolveMusicPath 三道防线对所有入口一致生效）。
...
// 首次启动（「本地音乐」歌单尚不存在）时，后台自动扫描 MUSIC_ROOT 并填充歌单。
// 已存在则跳过，避免覆盖用户已有的本地音乐歌单。挂载点未就绪时静默跳过。
(function autoScanLocalMusicOnBoot() {
  try {
    const created = !localLibrary.state.playlists.some(p => p.id === LOCAL_MUSIC_PLAYLIST_ID);
    ensureLocalMusicPlaylist();
    if (created) {
      scanDirectory('').then(scan => {
        const rawSongs = localMusicRawSongsFromScan(scan);
        localLibrary.replacePlaylistTracks(LOCAL_MUSIC_PLAYLIST_ID, rawSongs);
        console.log('[BootScan] 本地音乐扫描完成，共 ' + scan.total + ' 首' + (scan.truncated ? '（已截断）' : ''));
      }).catch(err => console.warn('[BootScan] 跳过（目录可能尚未挂载）：', err.message));
    }
  } catch (e) {
    console.warn('[BootScan] 初始化失败：', e.message);
  }
})();
